...
def add_transaction():
    ...
    # Deduct from the budget if expense
...
def view_budget_status():
    if not budgets:
        print_boxed(["No budgets set."])
        return

    lines = ["Budget Status:"]
    for category, budget in budgets.items():
        remaining = budget["limit"] - budget["spent"]
        lines.append(f"{category.capitalize()}: Limit = {budget['limit']}, Spent = {budget['spent']}, Remaining = {remaining}")
    print_boxed(lines)
